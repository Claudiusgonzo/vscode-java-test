...
public class JUnit5TestSearcher extends BaseFrameworkSearcher {
    ...
    @Override
    public TestItem parseTestItemFromMethodBinding(IMethodBinding methodBinding) throws JavaModelException {
        final TestItem item = super.parseTestItemFromMethodBinding(methodBinding);
    ...
    private boolean matchesNameInAnnotationHierarchy(IAnnotationBinding annotation, String annotationName, Set<ITypeBinding> hierarchy) {
        ...
        for (IAnnotationBinding annotationBinding : type.getAnnotations()) {
            ...
            if (annotationType != null && hierarchy.add(annotationType)) {
                if (matchesName(annotationType, annotationName) || matchesNameInAnnotationHierarchy(annotation, annotationName, hierarchy)) {
                    return true;
                }
            }
        }
